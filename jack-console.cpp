...
{   
    ...
    CMDUI2 interface(argc, argv, true);
    FUI finterface;
...
#endif
    auto handler_function = [&soundinterface] (std::string cmd) -> bool {
#ifdef SOUNDFILE_RELOAD
        soundinterface.printSoundfilesInfos();
        auto sf = soundinterface.createNewSoundfile(cmd);
        if (sf) {
    ...
    std::cout << "Type a path to a wav file to change on the fly" << '\n';
    interface.run(handler_function);
